...
スマホで公開URLを開き、ブラウザの共有メニューから「ホーム画面に追加」を選びます。

追加後、ホーム画面のアイコンから起動できるか確認してください。

Service Workerは静的ファイルをキャッシュします。更新が反映されない場合は、以下を順に試してください。
...
サイトデータ削除を行うとlocalStorageデータも消える可能性があります。削除前に必ずJSONバックアップを取ってください。
...
- 探索履歴キーは `kensho_campaign_discovery_history_v1` です。
...
探索履歴はキャンペーン本体とは別キーです。探索タブまたはバックアップタブから、探索履歴JSON/CSVを別途エクスポートできます。
...
- リマインダーが表示される
- 探索タブが表示される
- 検索キーワード生成と候補取り込みプレビューができる
- manifest.json が404にならない
- sw.js が404にならない
- PWAアイコンが404にならない
- ホーム画面に追加できる
- スマホ幅で崩れない
...
探索タブは検索キーワードと検索リンクを生成し、ユーザーが見つけた投稿URL・本文を手動で取り込む補助機能です。SNSの自動巡回、ログイン、応募、リアクション、DM確認は行いません。

ブラウザ通知は任意・許可制です。ICS出力はカレンダー予定ファイルを作るだけで、SNSや外部フォームは操作しません。

Service Workerは静的ファイルのキャッシュのみを行います。SNS操作、外部URLへの自動アクセス、投稿URLの自動オープン、自動応募は行いません。
