(function () {
  ...
  K.Risk.DANGER_PATTERNS = [
    ['クレジットカード情報を求めている', /クレジットカード|カード番号|セキュリティコード|決済情報/],
    ['銀行口座情報を求めている', /銀行口座|口座番号|振込先|キャッシュカード/],
    ['先払いを求めている', /先払い|前払い|事前入金|入金確認/],
    ['手数料を求めている', /手数料|登録料|参加費/],
    ['送料負担を求めている', /送料負担|送料|配送料/],
    ['LINE登録への誘導あり', /LINE登録|LINE追加|line\.me|lin\.ee/i],
    ['外部サイトへの誘導あり', /外部サイト|外部URL|別サイト|専用フォーム|応募フォーム|無料登録/],
    ['短縮URLあり', /短縮URL|bit\.ly|tinyurl|t\.co|x\.gd|is\.gd/i],
    ['強い煽り表現あり', /急いで|本日限定|今すぐ|必ず当たる|全員当選|絶対|限定公開/],
    ['個人情報入力が必要', /個人情報|住所|電話番号|氏名|配送先|本人確認|身分証/]
  ];
  K.Risk.CLEAR_DANGER_PATTERN = /クレジットカード|カード番号|セキュリティコード|銀行口座|口座番号|先払い|前払い|手数料|登録料|送料負担|必ず当たる|全員当選/i;
  ...
  K.Risk.evaluateRisk = function (campaign) {
    ...
    K.Risk.DANGER_PATTERNS.forEach(([reason, pattern]) => {
      ...
      reasons.push(reason);
      if (K.Risk.CLEAR_DANGER_PATTERN.test(body) && pattern.test(body)) {
        clearDanger = true;
        points += 35;
      } else {
        needsReview = true;
    ...
    if (/(100万円|10万円|高級車|海外旅行|iPhone|MacBook|現金)/.test(body) && !/(公式|認証|キャンペーン規約)/.test(body)) {
      points += 20; clearDanger = true; reasons.push('高額すぎる賞品の可能性');
    }
